...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
   ...
   is_admin_license = (
       license_data.plan == "lifetime_admin"
       or license_data.license_key.startswith("ADMIN-LIFETIME")
   )

   user = User(
       email=email,
       password_hash=hash_password(body.password),
       role="ADMIN" if is_admin_license else (license_data.role or "Owner"),
       active=True,
       is_staff=True if is_admin_license else False,
       owner_user_id=OWNER_USER_ID if is_admin_license else None,
   )
...
def me(
   ...
   return {
       ...
       "license": {
           ...
           "plan": license_data.plan if license_data else "Professional",
           "servers": "1 / 3",
           "expires": "Lifetime",
       }
   }